...
public class HexedMod extends Plugin{
    ...
    public void registerServerCommands(CommandHandler handler){
        ...
        handler.register("hexed14", "Begin hosting the Hexed gamemode with a map for max 14 players.", args -> {
            startHexed(290, 280);
        });


        handler.register("hexedcustom", "<w> <h>", "Begin hosting with the Hexed gamemode. (Custom map size)", args -> {
           startHexed(Strings.parseInt(args[0]), Strings.parseInt(args[1]));
    ...
    private void startHexed(int w, int h){
        chosen.clear();
        logic.reset();
        Log.info("Generating map...");
        world.loadGenerator(lastGenerator = new HexedGenerator(w, h));
        info("Map generated.");
        state.rules = rules.copy();
        logic.play();
        netServer.openServer();
        //block hail and ripple
        antilongturret();
    }

    private void antilongturret(){
        JsonValue base = JsonIO.json().fromJson(null, Core.settings.getString("globalrules"));
        JsonValue value = new JsonReader().parse("[hail, ripple]");
        value.name = "bannedBlocks";
        ...
        base.addChild("bannedBlocks", value);
